...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if (SONG == null)
			SONG = Song.loadFromJson('tutorial', true);
	...
	**/
	...
	function endSong():Void
	{
		...
		if (FlxG.state == instance)
		{
			...
			{
				#if !switch
				if (!hasUsedBot && songMultiplier >= 1 && utilities.Options.getData("customKeyAmount") <= 0)
				{
	...
	public function finishSongStuffs()
	{
		...
		if (isStoryMode)
		{
			...
			else
			{
				...
				PlayState.SONG = Song.loadFromJson(PlayState.storyPlaylist[0].toLowerCase() + difficulty, PlayState.storyPlaylist[0], true);
				FlxG.sound.music.stop();
	...
	public function returnStupidRatingText():String
	{
		...
		return ((utilities.Options.getData("marvelousRatings") == true ? "Marvelous: " + Std.string(ratingArray[0]) + "\n" : "")
			+ "Sick: "
			...
			+ Std.string(misses)
			+ "\n"
			+ (utilities.Options.getData("marvelousRatings") == true
				&& ratingArray[0] > 0
