...
import utilities.ManiaConverter;
...
class Song
{
	...
	public static function loadFromJson(jsonInput:String, ?folder:String, modify:Bool = false):SwagSong
	{
		...
		else
		{
			...
			parseJSONshit(rawJson, original_Folder);
		}
		
		if (modify)
		{
			final keyAmount = utilities.Options.getData("customKeyAmount");
			if (keyAmount > 0 && (song.keyCount != keyAmount || song.playerKeyCount != keyAmount))
				song = new ManiaConverter(song, keyAmount).convertedSong;
		}
