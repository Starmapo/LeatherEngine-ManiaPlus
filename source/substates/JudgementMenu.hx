...
class JudgementMenu extends MusicBeatSubstate
{
	...
	{
		judgementText.text = ("Preset: "
			+ preset
			+ (selected == 0 ? " <\n" : "\n")
			+ (utilities.Options.getData("marvelousRatings") ? "MARVELOUS: "
				+ Std.string(judgements[0])
				+ "ms"
				+ (selected == 1 ? " <\n" : "\n") : "")
			+ "SICK: "
			+ Std.string(judgements[1])
			+ "ms"
			+ (selected == (utilities.Options.getData("marvelousRatings") ? 2 : 1) ? " <\n" : "\n")
			+ "GOOD: "
			+ Std.string(judgements[2])
			+ "ms"
			+ (selected == (utilities.Options.getData("marvelousRatings") ? 3 : 2) ? " <\n" : "\n")
			+ "BAD: "
			+ Std.string(judgements[3])
			+ "ms"
			+ (selected == (utilities.Options.getData("marvelousRatings") ? 4 : 3) ? " <\n" : "\n")
			+ "\n");
